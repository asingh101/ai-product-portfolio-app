...
function createRoleAlignHandler({ db, geminiApiKey }) {
  return async (req, res) => {
    ...
    try {
      ...
      const config = await loadRoleAlignConfig(db);
      ...
      const body = req.body || {};
      if (!body.profile || !body.jobDescriptions?.length) {
        const err = {
          code: "VALIDATION",
          message: "Profile and job descriptions are required",
          retryable: false,
        };
        if (wantsSse) {
          writeSse(res, "error", err);
      ...
      console.error("[RoleAlign Error]", error);
      const errPayload = {
...
module.exports = { createRoleAlignHandler };
